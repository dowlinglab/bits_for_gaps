...
def _build(bounds=BOUNDS_2D, kernel=None, iters=1):
    return adaptiveEntropy(
        exp_name="validation_test", iters=iters, x_bounds=bounds, likelihood_var=0.05,
        mean_fxn=gpflow.mean_functions.Zero(), kernel_fxn=kernel or AnisotropicSE(),
...
@pytest.mark.parametrize("name,bad_value", [
    ("noIters", 0), ("noIters", -1), ("noIters", 1.5),
    ("noSamples", 0), ("noChains", 0), ("noLeapfrogSteps", 0),
    ("noGaussians", 0), ("noRestarts", 0),
    ("noBurnIn", -1), ("noAdaptSteps", -1),
])
...
def test_call_model_accepts_well_formed_black_box_output():
    s = _build()
    XData_new, yData_new = s.call_model([0.5, 358.0], np.array([[0.4, 357.0]]),
                                        np.array([1.0]))
