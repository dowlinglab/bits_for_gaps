...
Reads ``paper/golden/hyperparameter_posterior.json`` -- the posterior mean/median/std
...
def test_quantiles_bracket_median(golden):
    g = golden("hyperparameter_posterior.json")
    for p in PARAMS:
        assert g["q05"][p] < g["median"][p] < g["q95"][p]


def test_values_in_expected_ranges(golden):
    # Loose sanity envelope around the published posterior means; guards against a
    # golden regeneration that silently drifts far from the paper.
    g = golden("hyperparameter_posterior.json")
