...
Reads the committed golden snapshot (``paper/golden/fig5_error_metrics.json``) of the
...
def test_all_metrics_positive_and_finite(golden):
    g = golden("fig5_error_metrics.json")
    for it in ("iter_1", "iter_15"):
        for m in METRICS:
...
def test_quantiles_ordered(golden):
    g = golden("fig5_error_metrics.json")
    for it in ("iter_1", "iter_15"):
        for m in METRICS:
            s = g[it][m]
...
def test_test_error_drops_from_iter1_to_iter15(golden, metric):
...
def test_train_error_small_and_not_worse(golden):
